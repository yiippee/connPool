...
func NewConnPool(opt *Options) *ConnPool {
	p := &ConnPool{
		...
		// 通过一个chan来维护连接池的大小，因为如果还有空间则可以<-，否则会阻塞。
		queue:     make(chan struct{}, opt.PoolSize),
		// 已经创建的conn个数
		conns:     make([]*Conn, 0, opt.PoolSize),
		// put进池的conn个数，也就是用户程序释放的可重复利用的conn个数
		freeConns: make([]*Conn, 0, opt.PoolSize),
	}
	if opt.IdleTimeout > 0 && opt.IdleCheckFrequency > 0 {
		// 开启一个goroutine监视过期的连接
...
func (p *ConnPool) Get() (*Conn, bool, error) {
	...
	select {
	case p.queue <- struct{}{}: // 判断是否超过了pool的大小限制
	default:
		// 从定时器池里面获取一个定时器
		...
		case p.queue <- struct{}{}:
			...
			timers.Put(timer)
		case <-timer.C: // 超时了
			timers.Put(timer) // 将定时器放入定时器池中，便于下次使用
			atomic.AddUint32(&p.stats.Timeouts, 1) // 统计超时时间+1
	...
	for {
		...
		// 判断获取的连接是否过期，如果过期则关闭，并继续获取
		if cn.IsStale(p.opt.IdleTimeout) {
			p.CloseConn(cn)
			continue
		}

		atomic.AddUint32(&p.stats.Hits, 1) // 更新统计信息
		return cn, false, nil
	}
...
func (p *ConnPool) reaper(frequency time.Duration) {
	...
	for range ticker.C {
		...
		n, err := p.ReapStaleConns() // 周期性检查过期的连接
		if err != nil {
